...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        totalModification = 0
        inActs = [1.0] + inActs
        oldweights = list.copy(self.weights)
        for w in range(len(self.weights)):
            self.weights[w] += inActs[w] * alpha * delta
            totalModification += abs(self.weights[w] - oldweights[w])
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for i in range(len(self.layers)):
            lol.append([p.sigmoidActivation(lol[i]) for p in self.layers[i]])
        return lol
        ...
        for example in examples:  # for each example
            ...
            for layerNum in range(self.numHiddenLayers - 1, -1, -1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    delta = sum([nextLayer[i].weights[neuronNum + 1] * deltas[0][i]
                                 for i in range(len(nextLayer))]) * gPrime
            ...
            for numLayer in range(0, self.numLayers):
                ...
                for numNeuron in range(len(layer)):
                    weightMod = layer[numNeuron].updateWeights(allLayerOutput[numLayer], alpha,
                                                               deltas[numLayer][numNeuron])
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold=0.00008, hiddenLayerList=[1], maxItr=sys.maxsize,
    ...
    """
    backprop = nnet.backPropLearning(examplesTrain, alpha)
    while backprop[1] > weightChangeThreshold and iteration < maxItr:
        weightMod = weightMod + backprop[1]
        # if iteration % 10 == 0:
    ...
    for e in examplesTest:
        out = nnet.feedForward(e[0])
        temp = [round(i) for i in out[-1]]
        if temp == e[1]:
            testCorrect += 1
        else:
            testError += 1
