...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:
            emissionmodel = self.getObservationProb(observation, gameState.getPacmanPosition(),
                                                    pos, self.getJailPosition())
            counter[pos] = emissionmodel * self.beliefs[pos]
    ...
    def elapseTime(self, gameState):
        ...
        """
        counter = util.Counter()
        for priorpos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, priorpos)
            for postpos, prob in newPosDist.items():
                counter[postpos] += prob * self.beliefs[priorpos]
        self.beliefs = counter
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = []
        toadd = self.numParticles // len(self.legalPositions)
        for pos in self.legalPositions:
            self.particles += [pos] * toadd
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in range(len(self.particles)):
            tmp[self.particles[i]] = tmp[self.particles[i]] + self.getObservationProb(observation,
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(len(self.particles)):
            ...
            newPosDist = newPosDist.sample()
            tmp.append(newPosDist)
        self.particles = tmp
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = []
        legal = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for i in range(len(self.particles)):
            ...
            for j in range(self.numGhosts):
                probArr.append(self.getObservationProb(observation[j], gameState.getPacmanPosition(),
                                                       self.particles[i][j], self.getJailPosition(j)))
            k = 1
            for prob in probArr:
                k *= prob
            tmp[self.particles[i]] += k
        if not any(tmp.values()):
            self.initializeUniformly(gameState)
        else:
            tmp.normalize()
